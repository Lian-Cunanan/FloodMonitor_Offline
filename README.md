...
graph TD
    ...
    subgraph "🧠 ESP32 Core"
        CPU1[🔧 Core 0<br/>Main Processing]
        CPU2[🔧 Core 1<br/>WiFi & Tasks]
        MEM[💾 Memory<br/>512KB SRAM]
        WIFI[📡 WiFi Module<br/>802.11 b/g/n]
    end
    
    subgraph "📡 Sensor Network"
        US[🌊 Ultrasonic<br/>GPIO 5,18]
        DHT[🌡️ DHT22<br/>GPIO 4]
        RAIN[🌧️ Rain Sensor<br/>GPIO 34]
        LDR[💡 Light Sensor<br/>GPIO 35]
    end
    
    subgraph "⚙️ Actuators"
        SERVO[🚪 Servo Motor<br/>GPIO 2]
        LED[💡 Status LEDs<br/>Built-in]
    end
    ...
    style CPU1 fill:#2563eb,stroke:#1d4ed8,stroke-width:3px,color:#fff
    style CPU2 fill:#10b981,stroke:#059669,stroke-width:2px,color:#fff
    style WIFI fill:#f59e0b,stroke:#d97706,stroke-width:2px,color:#fff
```
...
graph LR
    subgraph "👤 User Actions"
        ...
        VIEW[👁️ View Dashboard]
        CONTROL[⚙️ Control Gates]
        LOGS[📝 Check Logs]
    end
    ...
    style AUTH fill:#ef4444,stroke:#dc2626,stroke-width:2px,color:#fff
    style DASH fill:#10b981,stroke:#059669,stroke-width:2px,color:#fff
    style PROC fill:#2563eb,stroke:#1d4ed8,stroke-width:2px,color:#fff
```
